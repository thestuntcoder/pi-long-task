...
assert.match(runningWidget, /Pi Long Task: task_start/);
assert.match(runningWidget, /Focus: TODO 2 .* Wire Sidebar Rendering/);
assert.match(runningWidget, /Worker spend: \$0\.01/);
assert.equal(renderRequests, 1);
const overlayText = overlayComponent?.render(80).join("\n") ?? "";
assert.match(overlayText, /Task timeline/);
assert.match(overlayText, /Focus: TODO 2/);
...
assert.match(doneWidget, /Pi Long Task: done/);
assert.match(doneWidget, /Tasks: 2\/3 done/);
